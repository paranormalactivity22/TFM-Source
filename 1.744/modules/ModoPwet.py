...
class ModoPwet:
    ...
    def makeReport(self, playerName, T, comments):
        ...
        if player != None:
            ...
            self.client.sendServerMessage("<ROSE>[Modopwet]</ROSE> The player <BV>%s</BV> has been reported for <N>%s</N> in room [%s] (<a href='event:modopwetfollow#%s'><FC>Follow</FC></a> - <a href='event:modopwetwatch#%s'><FC>Watch</FC></a>)." % (playerName,self.ReportType(type),roomName, playerName, playerName), True)
                ...
                for i in reports:
                    ...
                    if self.client.modoPwetLangue == 'ALL' or v["language"] == self.client.modoPwetLangue:
                        ...
                        for name in v["reporters"]:
                            ...
                            p.writeUTF(r[1])
                            p.writeByte(r[0])
                            print(int(Utils.getSecondsDiff(r[2])/60))
