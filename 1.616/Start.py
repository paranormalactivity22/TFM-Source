#coding: utf-8
import requests, re, os, sys, json, time, random, sqlite3, pymysql, traceback, zlib, asyncio, urllib.request, binascii, configparser, IP2Location, threading, asyncio
...
#IPdatabase = IP2Location.IP2Location(os.path.join("data", "./include/IP2LOCATION-LITE-DB5.BIN"))
...
class Client:
    ...
    async def playRecord(self):
        packets = self.packet_to_play
        _code = 83122#self.playerCode
        for i, packet in enumerate(packets):
            if i > 0:
                await asyncio.sleep(packet[0])

            if packet[1][0] == 0:
                self.sendPacket([4,4], ByteArray().writeInt(_code).write(packet[1][1:]).toByteArray())
            else:
                self.sendPacket([4,9], ByteArray().writeInt(_code).writeByte(packet[1][1]).writeByte(0).toByteArray())
        await asyncio.sleep(1)
        self.replaying = False
        self.room.removeClient(self.room.bot)
    ...
    def connection_made(self, transport, bot=False):
        ...
        self.DailyQuest = DailyQuest(self, self.server)
        self.AntiCheat = AntiCheat(self, self.server)
        self.Records = Records(self, self.server)
    ...
    def parseString(self, packet):
        ...
        if not self.validatingVersion:
            if (C == Identifiers.recv.Informations.C and CC == Identifiers.recv.Informations.Correct_Version) and not (self.isClosed):
                ...
                lang, ckey = packet.readUTF(), packet.readUTF()
    ...
    def loginPlayer(self, playerName, password, startRoom):
        ...
        if self.server.checkConnectedAccount(playerName):
            ...
            if not self.isGuest and not playerName == "":
                ...
                if len(players) > 1:
                    ...
                    if rs:
                        ...
                        self.dailyQuest = list(map(str, filter(None, rs[83].split(",")))) if rs[83] != "" else [0, 0, 0, 1]
            ...
            if "@" not in playerName:
                if self.privLevel == -1:
                    self.sendPacket(Identifiers.old.send.Player_Ban_Login, ["The account has been permanently removed."])
                    self.transport.close()
                    return
                ...
                if startRoom.startswith("\x03[Tutorial]"):
                    if not self.isGuest:
                        self.DailyQuest.loadDailyQuest(True)
                else:
                    self.DailyQuest.loadDailyQuest(False)
                #self.DailyQuest.loadDailyQuest(True)
...
#        if not self.room.isTotemEditor and not self.room.isEditor and not self.room.isRacing and not self.room.isBootcamp and not self.room.isSurvivor and not self.room.isVillage and not self.room.isDefilante:
        ...
        if self.room.isBigdefilante:
            self.sendMessage("<V>[#]</V> <V>Welcome to</V> <BL>Bigdefilante</BL>\n<V>[#]</V> <V>To view records list type</V> '<BL>/defrecs</BL>'") 
        if self.room.isMeepRace:
            self.sendMessage("<V>[#]</V> <V>Welcome to</V> <BL>Meepracing</BL>.\n<V>[#]</V> <BL>Press spacebar to meep</BL>") 
        ...
        if self.room.porno:
            self.sendMessage("<ROSE>You want titles?\n<VP>wait for title events! <J>%s" %(self.playerName))
...
##            self.sendPacket([29, 25], struct.pack('!h', len('<font color="#E2E05A">Fastracing')) + '<font color="#E2E05A">Fastracing')
    ...
    def sendCorrectVersion(self, lang='en'):
        self.sendPacket(Identifiers.send.Correct_Version, ByteArray().writeInt(len(self.server.players)).writeUTF(lang).writeUTF(lang).writeInt(0).writeBoolean(False).toByteArray())
    ...
    def sendPlayerIdentification(self):
        permsCount = 0
        perms = ByteArray()
        privAuthorization = {0:-1, 1:-1, 2:-1, 3:-1, 4:-1, 5:13, 6:11, 7:5, 8:5, 9:10} #FC, MC, ARB, MD, ADM
        permsList = []
    ...
    def sendEnterRoom(self, roomName):
        ...
        self.sendPacket(Identifiers.send.Enter_Room, ByteArray().writeBoolean(found).writeUTF("*?" if self.isTribeOpen else roomName).writeUTF("int" if roomName.startswith("*") else self.langue).toByteArray())
    ...
    def sendPlayerDied(self):
        self.room.sendAll(Identifiers.old.send.Player_Died, ByteArray().writeInt(self.playerCode).writeByte(self.playerScore).toByteArray())
        self.hasCheese = False
        self.Packets.packets = []
                    ...
                    if self.room.getPlayerCountUnique() >= self.server.needToFirst and self.room.countStats and not self.isShaman and not self.canShamanRespawn:
                        ...
                        if self.room.isBigdefilante:
                            if int(self.room.getPlayerCount()) >= int(self.server.needToFirst):
                                if self.room.mapCode not in (-1, 31, 41, 42, 54, 55, 59, 60, 62, 89, 92, 99, 114, 801):
                                    try:
                                        CursorMaps.execute('select BDTime from Maps where code = ?', [self.room.mapCode])
                                        timeDB = CursorMaps.fetchone()
                                        if timeDB[0] == 0 or timeTaken < timeDB[0]:
                                            CursorMaps.execute('update Maps set BDTime = ?, BDTimeNick = ? where code = ?', [timeTaken, self.playerName, self.room.mapCode])
                                            for client in self.room.clients.values():
    ...
    def sendProfile(self, playerName):
        ...
        packet = ByteArray()
        if player != None:
            if player.isMapCrew and player.privLevel < 6:
            ...
            elif player.isFunCorpPlayer and player.privLevel < 5:
    ...
    def openCafeTopic(self, topicID):
        ...
        for rs in self.CursorCafe.fetchall():
            if rs["Status"] != 2 or self.privLevel >= 7 or rs["Name"] == self.playerName:
                if self.privLevel >= 7 and rs["Status"] == 0:
        ...
        if mode != 18:
            ...
            for checkRoom in self.server.rooms.values():
                if {1:checkRoom.isNormRoom, 3:checkRoom.isVanilla or checkRoom.porno, 8:checkRoom.isSurvivor or checkRoom.isOldSurvivor, 9:checkRoom.isRacing or checkRoom.isSpeedRace or checkRoom.isMeepRace,  11:checkRoom.isMusic, 2:checkRoom.isBootcamp, 10:checkRoom.isDefilante or checkRoom.isBigdefilante, 18:0, 16:checkRoom.isVillage}[mode] and checkRoom.community == self.langue.lower():
                    roomsCount += 1
                    packet.writeByte(0).writeUTF(self.langue.lower()).writeUTF(self.langue.lower()).writeUTF(checkRoom.roomName).writeShort(checkRoom.getPlayerCount()).writeUnsignedByte(checkRoom.maxPlayers).writeBoolean(checkRoom.isFuncorpRoomName)
                
            if roomsCount == 0:
                packet.writeByte(0).writeUTF(self.langue.lower()).writeUTF(self.langue.lower()).writeUTF(("" if mode == 1 else (modeInfo[0]).split(" ")[1]) + "1").writeShort(0).writeUnsignedByte(200).writeBoolean(False)
        #Minigames
        else:
            #minigames, privateMinigames, minigamesList, roomsList = ["#bigdefilante", "#meepracing", "#deathmatch", "#ffarace", "#fastracing"], [], dict(), dict()
            minigames = ["#bigdefilante", "#meepracing", "#deathmatch", "#ffarace", "#fastracing"]
            minigamesList = {}
            roomsList = {}
    ...
    def reloadAllModules(self):
        ...
        for player in self.server.players.values():
            self.ModoCommands = ModoCommands(self, self.server)
            self.modoPwet = ModoPwet(self, self.server)
            self.tribulle = Tribulle(self, self.server)
            self.Shop = Shop(self, self.server)
            self.Skills = Skills(self, self.server)
...
class Server(asyncio.Transport):
    def __init__(self):
        ...
        self.lastRecordID = int(self.config("ids.lastrecordid"))
    ...
    def updateConfig(self):
        ...
        self.configs("ids.lastrecordid", str(self.lastRecordID))
...
class Room:
    def __init__(self, server, name):
        ...
        elif roomNameCheck.startswith("#") and not roomNameCheck.startswith in ["#meepracing", "#fastracing", "#ffarace", "#deathmatch", "#bigdefilante", "#fly"]:
            ...
            if self.isBigdefilante:
                ...
                if rs[0] > 0:
                    if rs[0] > 100:
                        ...
                        t = rs[0] / 10.0
                    for player in self.clients.values():
                        player.sendMessage("[<V>#</V>] <V>"+str(rs[1])+"</V> <N>broken in second (<V>"+ str(t) +"</V><R>s</V>)")
                else:
                    for player in self.clients.values():
                        player.sendMessage("[<V>#</V>] </font> <N>(<V>@"+str(self.mapCode)+"</V>) there are no broken records on this map.</N>")
