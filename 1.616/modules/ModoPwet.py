...
class ModoPwet:
    ...
    def makeReport(self, playerName, type, comments):
        ...
        if player != None:
            ...
            self.client.sendmodServerMessage("<ROSE>[Modopwet]</ROSE> The player <BV>%s</BV> has been reported for <N>%s</N> in room [%s]" % (playerName,self.ReportType(type),roomName))
    ...
    def deleteReport(self,playerName,handled):
        if handled == 0: # [deleted]
            ...
            if self.getReportType(playerName) == "Hack":
                self.server.banPlayer(playerName, 24, "Hack", "~Modopwet", "iban")
            elif self.getReportType(playerName) == "Insults":
                self.server.mutePlayerIP(playerName, 1, "Insults", "~Modopwet")
            elif self.getReportType(playerName) == "Phishing":
                self.server.IbanPlayer(playerName, 360, "Phishing", "~Modopwet", "iban")
            elif self.getReportType(playerName) == "Spam / Flood":
                self.server.mutePlayerIP(playerName, 1, "Flood", "~Modopwet")
            else:
                pass
            self.server.reports[playerName]["state"] = "deleted"
            self.server.reports[playerName]["deletedby"] = self.client.playerName
