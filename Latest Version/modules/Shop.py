...
class Shop:
    ...
    def sendShopList(self, sendItems=True):
        ...
        emojiesShop = self.server.shopEmojies if sendItems else []
        packet.writeEncoded(len(emojiesShop))
        for item in emojiesShop:
            packet.writeEncoded(int(item["id"]))
            packet.writeEncoded(item["cheese"])
            packet.writeEncoded(item["fraise"])
            packet.writeByte(item["purchasable"])
        
        emojies = [] if self.client.shopEmojies == "" else list(filter(None, self.client.shopEmojies.split(',')))
        packet.writeEncoded(len(emojies))
        for _id in emojies:
            packet.writeEncoded(int(_id))
