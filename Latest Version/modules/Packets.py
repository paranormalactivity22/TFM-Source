    ...
    def __init_2(self):
        ...
        async def Enter_Room(self, community, roomName):
            if self.client.playerName in ["", " "]:
                ...
                if not roomName == self.client.roomName or not self.client.room.isEditor or not len(roomName) > 64 or not self.client.roomName == "%s-%s" %(self.client.langue, roomName):
                    ...
                    elif roomName.startswith("*\x03") and not (roomName == "*\x03" + self.client.tribeName):
                        return
        ...
        async def Chat_Message(self, message):
            ...
            if self.client.isGuest:
                ...
                self.client.sendClientMessage("You can't speak while you are watching somebody.", 1)
                return
        ...
        async def Text_Area_Callback(self, textAreaID, event): #################
            ...
            if event in ["privpanel_closePanel", "privpanel_username", "privpanel_select", "privpanel_selectpool", "privpanel_unselect", "privpanel_unselectpool", "privpanel_selectall", "privpanel_selectallpool", "privpanel_removepriv", "privpanel_addpriv"] and self.client.privLevel > 8:
                event = event.split(":")
                if event[0] != "priv":
                    return
                elif event[1] == "closePanel":
                    self.privPanel.closePanel()
                elif event[1] == "username":
                    self.lastPopupID += 1
                    self.room.addPopup(self.lastPopupID, 2, "<font color='#B8D995'>Target Player</font>", 235, 160, 300, False)
                elif event[1] == "select":
                    priv, _id = map(int, event[2:])
                    self.privPanel.selectPriv(priv, _id)
                elif event[1] == "_select":
                    priv, _id = map(int, event[2:])
                    self.privPanel.selectPriv(priv, _id, pool = True)
                elif event[1] == "unselect":
                    priv, _id = map(int, event[2:])
                    self.privPanel.selectPriv(priv, _id, True)
                elif event[1] == "_unselect":
                    priv, _id = map(int, event[2:])
                    self.privPanel.selectPriv(priv, _id, True, True)
                elif event[1] == "all":
                    self.privPanel.selectAll()
                elif event[1] == "_all":
                    self.privPanel.selectAll(True)
                elif event[1] in ["remove", "add"]:
                    self.privPanel.editPrivs(event[1])
                    self.privPanel.reloadPrivs()
        ...
        async def BotProtection(self): #############
            ...
            print(code, self.packet.readInt(), self.client.verifycoder)
           #st1 = packet.readUTF()
        ...
        async def Ranking(self):
            ...
            for str2 in ["CheeseCount","FirstCount","ShamanCheeses","RacingStats","BootcampCount","SurvivorStats","DefilanteStats"]:
                ...
                for rs in self.Cursor['users'].find().sort(str2,-1): #i dont think is good change it
        ...
        @self.packet(args=['readBoolean'])
        async def Keep_Shaman_STRM(self, canShaman):
            if canShaman:
                self.client.room.forceNextShamanSTRM = self.client.playerCode
            else:
        ...
        async def Purchase_Emojies(self, emoji_id, fraises):
            ...
            amount = 0
            for emoji in shopEmojies:
                if emoji["id"] == emoji_id:
                    amount = int(emoji["fraise"]) if fraises else int(emoji["cheese"])
            if fraises:
