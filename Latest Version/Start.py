...
# Functions
    def sendModerationCommands(self):
        ...
        message += "<J>/del</J> <V>(mapName)</V> : <BL> Sets the given map on category p44 (Deleted).</BL>\n"
        ...
        if self.privLevel > 8:
            ...
            message += "<J>/baniperm</J> <V>[IP]</V> : <BL>Bans permanently the given ip address.</BL>\n"
            ...
            message += "<J>/debaniperm</J> <V>[IP]</V> : <BL>Unbans the selected IP address which was permanently banned. (Aliases: /unbaniperm)</BL>\n"
            message += "<J>/deluser</J> <V>[name]</V> : <BL>Deletes given user from the database.</BL>\n"
            ...
            message += "<J>/resetrecords</J> : <BL>Resets the records database.</BL>\n"
    ...
    def sendBulle(self):
        if self.room.isTribeHouse:
            self.room.bulle_id = 8
            
        elif self.room.bulle_id == 0:
            self.room.bulle_id = random.randint(0, 7)
            
        timestamp = int(time.time() / 100)
        self.sendPacket(Identifiers.send.Bulle, ByteArray().writeInt(self.room.bulle_id).writeInt(timestamp).writeInt(self.playerID if not self.isGuest else 0).writeUTF("127.0.0.1").writeUTF("11801-12801-13801-14801").toByteArray())
            
    def enterRoom(self, roomName):
        if roomName.startswith("*\x03") and not roomName == "*\x03" + self.tribeName and not roomName[2:] in self.invitedTribeHouses:
    ...
    async def loginPlayer(self, playerName, password, startRoom):
        ...
        if not self.canLogin[2]:
            print("[%s] [FATAL] Player tried login with an AIOTFM (%s)" %(time.strftime("%H:%M:%S"), playerName))
            self.transport.close()
            return
        ...
        if self.server.checkConnectedAccount(playerName):
            ...
            if "@" not in playerName:
                ...
                if not self.isGuest:
                    ...
                    _list = list(filter(None, self.shopEmojies.split(',')))
                    p = ByteArray().writeEncoded(len(_list))
                    for i in _list: p.writeEncoded(int(i))
                    self.sendPacket(Identifiers.send.Emoji_panel, p.toByteArray())
                ...
                if self.isGuest:
                    self.sendPacket(Identifiers.send.Emoji_panel, ByteArray().writeEncoded(0).toByteArray())
    ...
    def startPlay(self): ###########
        ...
        if self.isShaman and not self.room.roomDetails[1]:
        ...
        if mode == 18:
            ...
            for minigame in minigames:
                ...
                for checkRoom in self.server.rooms.values():
                    ...
                    if checkRoom.roomName.startswith(minigame):
                        roomsList[checkRoom.roomName] = [checkRoom.getPlayerCount(), checkRoom.roomDetails[8], checkRoom.isFuncorpRoomName]
            ...
            for checkRoom in self.server.rooms.values():
                if ({1:checkRoom.isNormRoom, 3:checkRoom.isVanilla, 8:checkRoom.isSurvivor, 9:checkRoom.isRacing or checkRoom.isFastRacing, 11:checkRoom.isMusic, 2:checkRoom.isBootcamp, 10:checkRoom.isDefilante, 18:0, 16:checkRoom.isVillage}[mode]):
                    ...
                    packet.writeByte(0).writeUTF(self.langue.lower() if not "*" in checkRoom.roomName else "int").writeUTF(self.langue.lower() if not "*" in checkRoom.roomName else "int").writeUTF(checkRoom.roomName).writeShort(checkRoom.getPlayerCount()).writeUnsignedByte(checkRoom.roomDetails[8]).writeBoolean(checkRoom.isFuncorpRoomName).writeBoolean(checkRoom.roomDetails[0])
                    if checkRoom.roomDetails[0]:
                        packet.writeBoolean(checkRoom.roomDetails[1]).writeBoolean(checkRoom.roomDetails[2]).writeBoolean(checkRoom.roomDetails[3]).writeBoolean(checkRoom.roomDetails[4]).writeBoolean(checkRoom.roomDetails[5]).writeByte(checkRoom.roomDetails[6]).writeInt(checkRoom.roomDetails[7]).writeShort(checkRoom.roomDetails[8]).writeByte(len(checkRoom.roomDetails[9]))
                        for i in checkRoom.roomDetails[9]: packet.writeByte(i)
    ...
    def sendModInfo(self, isOnline):
        if self.privLevel == 9:
            ...
            cc = "<font color='#b993ca'>" if not self.privLevel == 8 else "<font color='#c565fe'>" # b993ca
            ...
            for player in self.server.players.values():
                if player.playerName != self.playerName:
                    if player.privLevel == 9 and self.privLevel == 9:
                        self.sendMessage(f"<font color='#fc0303'>• [{player.langue.upper()}] {player.playerName} : {player.roomName}</font>")
                    
                    elif player.privLevel == 8 and self.privLevel == 8:
                        self.sendMessage(f"<font color='#b993ca'>• [{player.langue.upper()}] {player.playerName} : {player.roomName}</font>")
                    
                    elif (player.privLevel == 7 or player.isArbitre) and (self.privLevel == 7 or self.isArbitre):
                        self.sendMessage(f"<font color='#c565fe'>• [{player.langue.upper()}] {player.playerName} : {player.roomName}</font>")
                    
                    elif (player.privLevel == 6 or player.isMapCrew) and (self.privLevel == 6 or self.isMapCrew):
                        self.sendMessage(f"<font color='#2F7FCC'>• [{player.langue.upper()}] {player.playerName} : {player.roomName}</font>")
                    
                    elif (player.privLevel == 5 or player.isFunCorpPlayer) and (self.privLevel == 5 or self.isFunCorpPlayer):
                        self.sendMessage(f"<font color='#F89F4B'>• [{player.langue.upper()}] {player.playerName} : {player.roomName}</font>")
                    
                    elif (player.privLevel == 4 or player.isLuaCrew) and (self.privLevel == 4 or self.isLuaCrew):
                        self.sendMessage(f"<font color='#79bbac'>• [{player.langue.upper()}] {player.playerName} : {player.roomName}</font>")
                    
                    elif (player.privLevel == 3 or player.isFashionSquad) and (self.privLevel == 3 or self.isFashionSquad):
                        self.sendMessage(f"<font color='#ffb6c1'>• [{player.langue.upper()}] {player.playerName} : {player.roomName}</font>")
        ...
        if timeTaken > 7:
            ...
            if canGo:
                ...
                if place == 1: ##############
                    ...
                    if (self.room.getPlayerCountUnique() >= self.server.needToFirst and self.room.countStats and not self.isShaman and not self.canShamanRespawn and not self.isGuest) or self.server.isDebug:
                        self.firstCount += 1
                ...
                elif self.room.getPlayerCountUnique() >= self.server.needToFirst and self.room.isBootcamp and not self.isGuest:
...
class Server(asyncio.Transport):
    ...
    def checkRoom(self, roomName, langue):
        ...
        if (("%s-%s" %(langue, roomName)) if not roomName.startswith("*") and not roomName.startswith("@") and roomName[0] != chr(3) else roomName) in self.rooms:
            ...
            if room.getPlayerCount() < room.roomDetails[8] if room.roomDetails[8] != 0 else True:
                found = True
        else:
            found = True
    ...
    def loadShopList(self):
        ...
        self.shopEmojies = self.shopData["emojies"]
    ...
    def tempBanIP(self, ip, time):
        if not ip in self.IPTempBanCache:
            ...
            self.Cursor['iptempban'].insert_one({'Ip':ip})
            if ip in self.IPTempBanCache:
                self.loop.call_later(time, lambda: self.removeTempBanIP(ip))

    def removeTempBanIP(self, ip):
        self.Cursor['iptempban'].delete_one({'Ip':ip})
        self.IPTempBanCache.remove(ip)
...
class Room:
    ...
    async def selectMapStatus(self):
        ...
        selectPerma = random.choice(self.roomDetails[9]) if len(self.roomDetails[9]) != 0 else (17 if self.mapStatus % 2 == 0 else 7) if self.isRacing or self.isFastRacing else (13 if self.mapStatus % 2 == 0 else 3) if self.isBootcamp else 18 if self.isDefilante else (11 if self.mapStatus == 0 else 10) if self.isSurvivor else 19 if self.isMusic and self.mapStatus % 2 == 0 else 0
        isMultiple = False
    ...
    def getDoubleShamanCode(self):
        if self.currentShamanCode == -1 and self.currentSecondShamanCode == -1:
            if self.forceNextShamanSTRM > 0:
                self.currentShamanCode = self.forceNextShamanSTRM
                self.forceNextShamanSTRM = 0
                
            elif self.forceNextShaman > 0:
    ...
    def giveDefilanteStats(self, increment=1):
        if not self.countStats:
            return
        for player in self.clients.copy().values():
            if not player.isNewPlayer and not player.isGuest:
    ...
    def giveShamanSave(self, shamanName, type):
        ...
        if player != None and (self.getPlayerCountUnique() >= self.server.needToShamanPlayers or self.server.isDebug) and not player.isGuest:
            if type == 0:
...
# Lua
    ...
    def spawnNPC(self, npcName, data={}): # FIX NPCS
        self.sendAll(Identifiers.send.NPC, ByteArray().writeInt(int(data["id"]) if "id" in data else 0).writeUTF(npcName).writeShort(int(data["title"]) if "title" in data else 0).writeUTF(data["look"] if "look" in data else "").writeShort(int(data["x"]) if "x" in data else 0).writeShort(int(data["y"]) if "y" in data else 0).writeBoolean(bool(data["female"]) if "female" in data else 0).writeBoolean(bool(data["lookLeft"]) if "lookLeft" in data else 0).writeBoolean(bool(data["starePlayer"]) if "starePlayer" in data else False).writeBoolean(bool(data["interactive"]) if "interactive" in data else False).toByteArray())
