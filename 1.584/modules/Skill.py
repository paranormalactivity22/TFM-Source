...
class Skills:
    ...
    def sendSkillObject(self, objectID, posX, posY, angle):
        self.client.room.sendAll(Identifiers.send.Skill_Object, ByteArray().writeShort(posX).writeShort(posY).writeByte(objectID).writeShort(angle).toByteArray())
    ...
    def sendGravitationalSkill(self, seconds, velX, velY):
        self.client.room.sendAll(Identifiers.send.Gravitation_Skill, ByteArray().writeShort(seconds).writeInt(velX).writeInt(velY).toByteArray())
