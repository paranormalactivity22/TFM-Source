...
class Lua:
    ...
    def disableWatchCommand(self, status=False):
        self.room.sendAll(Identifiers.send.Lua_Disable, ByteArray().writeBoolean(status).writeBoolean(0).writeBoolean(0).toByteArray())
        
    def disableDebugCommand(self, status=False):
        self.room.sendAll(Identifiers.send.Lua_Disable, ByteArray().writeBoolean(0).WriteBoolean(status).toByteArray())
    
    def disableMinimalistMode(self, status=False):
        self.room.sendAll(Identifiers.send.Lua_Disable, ByteArray().writeBoolean(0).WriteBoolean(0).WriteBoolean(status).toByteArray())
