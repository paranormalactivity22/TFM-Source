...
##            self.sendPacket([29, 25], struct.pack('!h', len('<font color="#E2E05A">Fastracing')) + '<font color="#E2E05A">Fastracing')
    ...
    def sendProfile(self, playerName):
        ...
        if player != None and not player.isGuest:
            packet = ByteArray().writeUTF(player.playerName).writeInt(player.playerID).writeInt(str(player.regDate)[:10]).writeInt(int({1:"009D9D",2:"009D9D",3:"009D9D",4:"009D9D",5:"F89F4B",6:"2F7FCC",7:"009D9D",8:"BABD2F",9:"EB1D51"}[player.privLevel],16)).writeByte(player.gender).writeUTF(player.tribeName).writeUTF(player.marriage)
            ...
            for checkRoom in self.server.rooms.values():
                if {1:checkRoom.isNormRoom, 3:checkRoom.isVanilla, 8:checkRoom.isSurvivor, 9:checkRoom.isRacing or checkRoom.isSpeedRace,  11:checkRoom.isMusic, 2:checkRoom.isBootcamp, 10:checkRoom.isDefilante, 18:0, 16:checkRoom.isVillage}[mode] and checkRoom.community == self.langue.lower():
                    ...
                    if checkRoom.isFuncorp == True and checkRoom.isFuncorpRoomName == False:
                        packet.writeByte(0).writeUTF(self.langue.lower()).writeUTF(self.langue.lower()).writeUTF(checkRoom.roomName).writeShort(checkRoom.getPlayerCount()).writeUnsignedByte(checkRoom.maxPlayers).writeBoolean(checkRoom.isFuncorp)
                    else:
                        packet.writeByte(0).writeUTF(self.langue.lower()).writeUTF(self.langue.lower()).writeUTF(checkRoom.roomName).writeShort(checkRoom.getPlayerCount()).writeUnsignedByte(checkRoom.maxPlayers).writeBoolean(checkRoom.isFuncorp)
                        
            if roomsCount == 0:
                packet.writeByte(0).writeUTF(self.langue.lower()).writeUTF(self.langue.lower()).writeUTF(("" if mode == 1 else (modeInfo[0]).split(" ")[1]) + "1").writeShort(0).writeByte(200).writeBoolean(False)
        #Minigames
        else:
            ...
            for minigame, count in minigamesList.items():
                packet.writeByte(1).writeUTF(self.langue.lower()).writeUTF(self.langue.lower()).writeUTF(str(minigame)).writeUTF(str(count)).writeUTF("mjj").writeUTF((minigame + self.playerName.lower() if minigame == "#utility" else minigame))

            for minigame, count in roomsList.items():
                packet.writeByte(0).writeUTF(self.langue.lower()).writeUTF(self.langue.lower()).writeUTF(str(minigame)).writeShort(count[0]).writeUnsignedByte(count[1]).writeBoolean(False)
...
class Server(asyncio.Transport):
    def __init__(self):
        ...
        self.ac_enabled = self.activateAntiCheat
