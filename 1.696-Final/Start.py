...
# Packets
    ...
    def connection_made(self, transport):
        ...
        self.GameException = GameException(self)
    ...
    async def parseString(self, packet):
        ...
        if not self.validatingVersion:
            if (C == Identifiers.recv.Informations.C and CC == Identifiers.recv.Informations.Correct_Version):
...
# Functions
    ...
    def giveConsumable(self, id, amount, limit=80, flag=0):
        if flag:
            ...
            if not self.isGuest and not playerName == "":
                ...
                if len(players) > 1:
                    ...
                    if rs:
                        ...
                        self.shamanSavesNoSkill = rs[11]
                        self.hardModeSaves = rs[12]
                        self.hardModeSavesNoSkill = rs[13]
                        self.divineModeSaves = rs[14]
                        self.divineModeSavesNoSkill = rs[15]
                        self.bootcampCount = rs[16]
                        self.shamanType = rs[17]
                        self.shopItems = rs[18]
                        self.shamanItems = rs[19]
                        self.clothes = list(map(str, filter(None, rs[20].split("|")))) ####
                        self.playerLook = rs[21]
                        self.shamanLook = rs[22]
                        self.mouseColor = rs[23]
                        self.shamanColor = rs[24]
                        self.regDate = rs[25]
                        self.shopBadges = list(map(int, filter(None, rs[26].split(","))))
                        self.cheeseTitleList = list(map(float, filter(None, rs[27].split(","))))
                        self.firstTitleList = list(map(float, filter(None, rs[28].split(","))))
                        self.shamanTitleList = list(map(float, filter(None, rs[29].split(","))))
                        self.shopTitleList = list(map(float, filter(None, rs[30].split(","))))
                        self.bootcampTitleList = list(map(float, filter(None, rs[31].split(","))))
                        self.hardModeTitleList = list(map(float, filter(None, rs[32].split(","))))
                        self.divineModeTitleList = list(map(float, filter(None, rs[33].split(","))))
                        self.specialTitleList = list(map(float, filter(None, rs[34].split(","))))
                        self.banHours = rs[35]
                        self.shamanLevel = rs[36]
                        self.shamanExp = rs[37]
                        self.shamanExpNext = rs[38]
                        for skill in list(map(str, filter(None, rs[39].split(";")))):
                            values = skill.split(":")
                            self.playerSkills[int(values[0])] = int(values[1])

                        self.lastOn = rs[40]
                        self.friendsList = rs[41].split(",")
                        self.ignoredsList = rs[42].split(",")
                        self.gender = rs[43]
                        self.lastDivorceTimer = rs[44]
                        self.marriage = rs[45]
                        ...
                        for consumable in list(map(str, filter(None, rs[54].split(";")))):
                            values = consumable.split(":")
                            self.playerConsumables[int(values[0])] = int(values[1])
                            
                        self.equipedConsumables = rs[55] #rs[52]
                        self.pet = rs[56]
                        self.petEnd = 0 if self.pet == 0 else Utils.getTime() + rs[57]
                        self.fur = rs[58]
                        self.furEnd = 0 if self.furEnd == 0 else Utils.getTime() + rs[59]
                        self.shamanBadges = list(map(int, filter(None, rs[60].split(","))))
                        self.equipedShamanBadge = rs[61]
                        self.totem = [rs[62], rs[63].replace("%"[0], chr(1))]
                        self.visuDone = rs[64].split("|")
                        self.custom = list(map(str, filter(None, rs[65].split(","))))
                        for counts in map(str, filter(None, rs[67].split(';'))):
                            values = counts.split(':')
                            f = []
                            aux = 0
                        ...
                        for points in list(map(str, filter(None, rs[68].split(';')))):
                            values = points.split(':')
        ...
        if mode == 18:
            ...
            for checkRoom in self.server.rooms.values():
                if ({1:checkRoom.isNormRoom, 3:checkRoom.isVanilla, 8:checkRoom.isSurvivor, 9:checkRoom.isRacing or checkRoom.isFastRacing, 11:checkRoom.isMusic, 2:checkRoom.isBootcamp, 10:checkRoom.isDefilante, 18:0, 16:checkRoom.isVillage}[mode]):
                    roomsCount += 1
                    packet.writeByte(0).writeUTF(self.langue.lower()).writeUTF(self.langue.lower()).writeUTF(checkRoom.roomName).writeShort(checkRoom.getPlayerCount()).writeUnsignedByte(checkRoom.maxPlayers).writeBoolean(checkRoom.isFuncorpRoomName).writeByte(0)
    ...
    def sendModMuteMessage(self, playerName, hours, reason, only):
        if only == False:
            self.sendLangueMessage("", "<ROSE>$MuteInfo2", playerName, playerName, hours, reason, True)
        else:
    ...
    def sendServerMessage(self, message, modopwetNotifications=False):
        for client in self.server.players.values():
            if client.privLevel >= 7 and not modopwetNotifications:
                client.sendPacket(Identifiers.send.Recv_Message, ByteArray().writeByte(0).writeUTF(message).writeShort(0).toByteArray())
            elif client.privLevel >= 7 and client.isModoPwetNotifications and modopwetNotifications:
    ...
    def tradeInvite(self, playerName):
        player = self.room.clients.get(playerName)
        if player != None and (not self.ipAddress == player.ipAddress) and self.privLevel > 0 and player.privLevel > 0:
...
class Server(asyncio.Transport):
    ...
    def loadRecords(self, showConsole = 1):
    ...
    def mutePlayer(self, playerName, hours, reason, modName, isOriginal=False, isSilent=False):
        ...
        if player != None:
            ...
            player.isMute = True
            if isOriginal == True:
                player.sendModMuteMessage(playerName, hours, reason, not isSilent)
                player.sendModMuteMessage(playerName, hours, reason, True)
            self.userMuteCache.append(playerName)
            Cursor.execute("insert into UserTempMute values (%s, %s, %s)", [playerName, str(Utils.getTime() + (hours * 60 * 60)), reason])
    ...
    def saveCasier(self, playerName, state, bannedby, time, reason=""):   
        Cursor.execute("insert into casierlog values (%s, %s, %s, %s, %s, %s, %s)", [playerName, self.getPlayerIP(playerName), state, Utils.getTime(), bannedby, time, reason]) 
    ...
    def sendServerRestartSEC(self, seconds):
        ...
        if seconds <= 1:
            Cursor.execute("DELETE FROM loginlogs")
            Cursor.execute("DELETE FROM commandlog")
    ...
    def reloadServer(self):
        ...
        for player in self.players.copy().values():
            ...
            player.GameException = _module.GameException(player)
...
class Room:
    ...
    def mapChange(self):
        ...
        if self.initVotingMode:
            ...
            if player in self.clients.copy().values():
                if player.pet != 0:
    ...
    def selectMap(self):
        if not self.forceNextMap == "-1":
            ...
            elif self.isVillage or self.is801Room:
                return 801
