...
# Arbitre Commands:           
                        ...
                        if self.client.privLevel >= 7:
                            ...
                            if result == -1:
                                ...
                                if self.server.checkConnectedAccount(playerName):
                                    ...
                                    self.client.sendServerMessage(f"{self.client.playerName} banned the player {playerName} for {hours}h ({reason}).")
                    ...
                    try:
                        if args[0] == "list" and self.requireArguments(1):
                            ...
                            if not name in self.server.serverList:
                                self.client.sendClientMessage("The string <N>[%s]</N> is not in the filter." %(name), 1)
                            else:
                                self.server.serverList.remove(name)
                                self.client.sendClientMessage("The string <N>[%s]</N> has been removed from the filter." %(name), 1)
                            ...
                            if name in self.server.serverList:
                                self.client.sendClientMessage("The string <N>[%s]</N> is already on the filter." %(name), 1)
                            else:
                ...
                if self.client.privLevel >= 7:
                    if len(args) == 0:
                        ...
                        for player in [*self.client.room.clients.copy().values()]:
                            player.enterRoom('1')
                        self.client.sendServerMessage(str(self.client.playerName)+" closed the room ["+roomName+"].")
                    elif len(args) == 1:
...
# Moderator Commands
                ...
                if self.client.privLevel >= 8 and self.requireArguments(2):
                    ...
                    if player != None:
                        ...
                        self.client.sendClientMessage("The supplied argument isn't a valid nickname.")
                ...
                if self.client.privLevel >= 8 and self.requireArguments(1):
                    ...
                    if playerName in t["records"]:
                        ...
                        self.client.sendServerMessage("The %s's records was removed by %s." %(playerName, self.client.playerName))
...
# Admins Commands
                ...
                if self.client.privLevel >= 9:
                    ...
                    if playerName in self.client.room.clients:
                        ...
                        self.client.sendClientMessage("Successfull respawned "+str(playerName)+".", 1)
                ...
                if self.client.privLevel >= 9 and self.requireArguments(1):
                    ...
                    self.client.sendClientMessage("Successfull added "+str(time)+" seconds to current round.", 1)
                ...
                if self.client.privLevel >= 9 and self.requireArguments(2):
                    ...
                    if code.isdigit():
                        ...
                        self.client.sendClientMessage("The code must be integer.", 1)
                ...
                if self.client.privLevel >= 9:
                    if len(args) == 1:
                        ...
                        if mapCode != -1:
                            ...
                            self.client.sendClientMessage("Successfull deleted the map: @"+str(mapCode)+" from database.", 1)
                    elif len(args) == 0:
                        mapCode = self.client.room.mapCode
                        if mapCode != -1:
                            self.client.room.CursorMaps.execute("delete from Maps where Code = ?", [mapCode])
